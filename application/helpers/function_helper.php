...
function line_notify_message($type = "4", $message, $line_notify_id = null) {
    ...
    */
    $CI = & get_instance();
    if ($type == "4") {
        $line_notify_status = $CI->Setting_model->setting_find(['name' => 'line_notify_log_api_status'
        // Old setting for AK !!!
        ]);
    } else {
        $line_notify_status = $CI->Setting_model->setting_find(['name' => 'line_notify_status'
        // Current settings
        ]);
    }
    // $message_new = $message;
    ...
    $manual_linenoti_other_log = $CI->Setting_model->setting_find(['name' => 'manual_linenoti_other_log']); // รับสถานะแจ้งรายงานผล log อื่นๆ
    $manual_linenoti_register = $CI->Setting_model->setting_find(['name' => 'manual_linenoti_register']); // รับสถานะแจ้งสมัคร
    if ($line_notify_status != "" && $line_notify_status['value'] == "1") {
        if ($type == "4") {
            $line_notify_token = $CI->Setting_model->setting_find(['name' => 'line_notify_log_api_token']);
        } else {
            $line_notify_token = $CI->Setting_model->setting_find(['name' => 'line_notify_token']);
        }
        if ($line_notify_token != "" && !empty($line_notify_token['value'])) {
            ...
            try {
                ...
                if (!is_null($line_notify_id)) {
                    $log_line_notify_id = $CI->Log_line_notify_model->log_line_notify_update(['id' => $line_notify_id, 'message' => $message_new, 'response' => json_encode(['headers' => $header, 'response' => $response, ], JSON_UNESCAPED_UNICODE), 'type' => $type, 'status' => 1, ]);
                } else {
                    $log_line_notify_id = $CI->Log_line_notify_model->log_line_notify_create(['message' => $message_new, 'response' => json_encode(['headers' => $header, 'response' => $response, ], JSON_UNESCAPED_UNICODE), 'type' => $type, 'status' => 1, ]);
                }
                return ['headers' => $header, 'response' => $response, ];
            }
            catch(Exception $ex) {
                if (!is_null($line_notify_id)) {
                    $log_line_notify_id = $CI->Log_line_notify_model->log_line_notify_update(['id' => $line_notify_id, 'message' => $message_new, 'status' => 1, 'response' => json_encode(["status" => false, "Error : " . $ex->getMessage() ], JSON_UNESCAPED_UNICODE), 'type' => $type]);
                } else {
                    $log_line_notify_id = $CI->Log_line_notify_model->log_line_notify_create(['message' => $message_new, 'status' => 1, 'response' => json_encode(["status" => false, "Error : " . $ex->getMessage() ], JSON_UNESCAPED_UNICODE), 'type' => $type]);
                }
            ...
            if (!is_null($line_notify_id)) {
                ...
                $log_line_notify_id = $CI->Log_line_notify_model->log_line_notify_create(['message' => $message_new, 'status' => 1, 'response' => json_encode(["status" => false, "Line Notify token : Empty"], JSON_UNESCAPED_UNICODE), 'type' => $type]);
            }
            return ['response' => ["status" => false, "Line Notify token : Empty"], ];
        }
    }
    if (!is_null($line_notify_id)) {
        $log_line_notify_id = $CI->Log_line_notify_model->log_line_notify_update(['id' => $line_notify_id, 'message' => $message_new, 'status' => 1, 'response' => json_encode(["status" => false, "Line Notify status : Disabled"], JSON_UNESCAPED_UNICODE), 'type' => $type]);
    } else if (is_null($line_notify_id) && !empty($message_new)) {
        $log_line_notify_id = $CI->Log_line_notify_model->log_line_notify_create(['message' => $message_new, 'status' => 1, 'response' => json_encode(["status" => false, "Line Notify status : Disabled"], JSON_UNESCAPED_UNICODE), 'type' => $type]);
    }
    return ['response' => ["status" => false, "Line Notify status : Disabled"], ];
}
