														...
														if($check == 0 && $check_add_minute_once == 0){
															...
															if(count($split_statement_detail) > 1 && array_key_exists(trim($split_statement_detail[0]),$bank_stmt_list)){
																...
																if(!empty($bank_in) && count($bank_in) > 0 && !empty($bank_number_like)){
																	$sql_acc_check = "SELECT id,amount_deposit_auto,bank,bank_number,bank_name,username FROM `account` where bank IN ('".implode("','",$bank_in)."') and bank_number like '".$bank_number_like."' and deleted = '0' ORDER BY active_deposit_date DESC";
