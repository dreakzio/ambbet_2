...
class ScbLine {
	...
	private $cookieJarFilePath = 'cookies/';
...
// The second parameter should be the path of your destination
    ...
    $d = json_decode($res,true);
		...
		if($d['status']['code'] == 1000) {
			...
			$a = $d['data']['monthInfo'];
			$last = end($a);
			$data['availableBalance'] = $last['inflowAmount'] - $last['spendingAmount'];
			$data['status'] = 'Success';
			// $data['data'] = $d['data'];
	...
	public function getTransaction() {
		...
		if($d['status']['code'] == 1000) {
			...
			for($i=0, $iMax = count($d['data']['txnDetails']); $i< $iMax; $i++){
				...
				$detail =$this->getDetailTransection($this->accno,$this->cookie,$date,$d['data']['txnDetails'][$i]['inflow'][0]['recordId']);
				//print_r($detail);
				//sleep(10);
				preg_match_all ("/SCB x(.*) /U", $detail['data']['txnDetail']['accountFrom'], $scbbank);
				preg_match_all ("/ ((.*)) \/X([0-9]+)([0-9]+)([0-9]+)([0-9]+)([0-9]+)([0-9]+)/U", $detail['data']['txnDetail']['accountFrom'], $otherbank);
				$bankno = "";
				if($scbbank[0]){
				...
				$data['deposit'][$i]['description_full'] = $detail['data']['txnDetail']['accountFrom'];
