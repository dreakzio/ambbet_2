...
class Transfer_marketing_model extends CI_Model
{
	...
	public function getRefByAccountId_2($account_id_list = []){
		...
		$this->db->select('
			...
			agent
        ');
		$this->db->where_in('to_account', $account_id_list);
		$query = $this->db->get('ref');
		$ref_list = $query->result_array();
		$data = [];
		foreach($ref_list as $result){
			$data[$result['to_account']] = $result;
		}
		return $data;
