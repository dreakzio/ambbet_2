...
class LogLineNotify extends CI_Controller
{
    ...
    {
        $data['page'] = 'log/line_notify';
        $this->load->view('main', $data);
    }
    public function log_line_notify_list_page()
    {
        $get = $this->input->get();
        ...
        $log_count_all = $this->Log_line_notify_model->log_line_notify_count();
        $log_count_search = $this->Log_line_notify_model->log_line_notify_count($search_data);
        $data = $this->Log_line_notify_model->log_line_notify_list($search_data);
        echo json_encode([
