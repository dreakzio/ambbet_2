...
class User_suspend extends CI_Controller
{
	...
	public function index()
	{
		...
		$log_page_id = $this->Log_page_model->log_page_create([
			...
			'page_name' => "สมาชิกที่ถูกระงับ",
			'description' => 'หน้ารายการ',
			'page_url' => $currentURL,
		]);
		$data['page'] = 'user_suspend/user_suspend';
		$this->load->view('main', $data);
	}
	public function user_list_page()
	{
		$get = $this->input->get(); // declare for recieve data and catch values form url via GET Method
		...
		$user_count_all = $this->User_suspend_model->user_count();
		$user_count_search = $this->User_suspend_model->user_count($search_data);
		...
		$data = $this->User_suspend_model->user_list_page($search_data);
		...
		if(count($user_id_list) > 0){
			...
			foreach($user_id_list as $user_id){
				if(array_key_exists($user_id,$sum_amount_list)){
					$this->User_suspend_model->user_update(['sum_amount'=>$sum_amount_list[$user_id]['sum_amount'],'id'=> $user_id]);
				}
			}
	...
	public function user_form_update($id = "")
	{
		...
		$data['user'] = $this->User_suspend_model->user_find([
			'id' => $id
		]);
		if ($data['user']=="") {
			redirect('user_suspend');
		}
		...
		if(array_key_exists($data['user']['id'],$sum_amount_list)){
			$this->User_suspend_model->user_update(['sum_amount'=>$sum_amount_list[$data['user']['id']]['sum_amount'],'id'=> $data['user']['id']]);
			$data['user']['sum_amount'] = $sum_amount_list[$data['user']['id']]['sum_amount'];
		}
		if(!empty($data['user']['ref_from_account'])){
			$data['user_select'] = $this->User_suspend_model->user_select2([
		...
		$data['page'] = 'user_suspend/user_suspend_update';
		$this->load->view('main', $data);
	}
	public function user_update($id = "")
	{
		$user = $this->User_suspend_model->user_find([
			'id' => $id
		]);
		if ($user=="") {
			redirect('user_suspend');
		...
		$this->User_suspend_model->user_update($update);
		$this->Ref_model->ref_agent_user_update([
			'from_account' => $id,
			'agent' => $post['agent']
		]);
		if(isset($post['username_ref']) && !empty($post['username_ref'])){
			$username_ref = $this->User_suspend_model->user_find([
		...
		$username_ref_old = $this->User_suspend_model->user_find([
	...
	// function for load another 25 columns
	...
	// this function for create member by superrole
	public function user_create_agent_username($id = "")
	{
		...
		if(isset($response['code']) && $response['code'] == 0 && isset($response['result']) && isset($response['result']['loginName']) ){
			$this->User_suspend_model->user_update([
				'id' => $user['id'],
