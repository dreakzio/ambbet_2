...
function dataTable() {
	...
	table = $('#table').DataTable({
		...
		// 'serverMethod': 'get',
		ajax: {
			url: BaseURL + 'LogReturnBalance/log_return_balance_list_page',
			data: function(d) {
		...
		columns: [
			...
			},
			{
				...
				render: function(data, type, full, meta) {
					...
					let html = numeral(point_before).format('0,0.00')+" / "+numeral(point).format('0,0.00')+" / "+numeral(point_after).format('0,0.00');
					return html != "" && html != null ? html : "-";
				}
			},
			{
			...
			},
			{
				...
				render: function(data, types, full, meta) {
					...
					if(type == "0"){
						html = "<p class='mb-0 text-success'><i class='fa fa-arrow-up mr-50'></i>เพิ่มแต้ม</p>";
					}else if(type == "1"){
						html = "<p class='mb-0 text-danger'><i class='fa fa-arrow-down  mr-50'></i>ใช้แต้ม </p>";
					}
					return html;
				}
			},
