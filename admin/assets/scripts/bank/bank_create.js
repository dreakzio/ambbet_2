$(document).on('click', '#btn_create', function() {
	...
	if (username.trim().length == 0 && bank_code !='11') {
		$('#username').focus();
		sweetAlert2('warning', 'กรุณาระบุ Username');
		return false;
	}
	let password = $('#password').val();
	if (password.trim().length == 0  && bank_code !='11') {
		$('#password').focus();
	...
	if (api_token_1.trim().length == 0 &&
		($('#bank_code').val().toString().trim() == "05"
			|| $('#bank_code').val().toString().trim() == "5"
			|| $('#bank_code').val().toString().trim() == "02"
			|| $('#bank_code').val().toString().trim() == "2"
			|| $('#bank_code').val().toString().trim() == "03"
			|| $('#bank_code').val().toString().trim() == "3"
			|| $('#bank_code').val().toString().trim() == "10"
			|| $('#bank_code').val().toString().trim() == "11"
		)
	) {
		$('#api_token_1').focus();
...
$(document).on('change', '#bank_code', function(e) {
	...
	if(value == "05" || value == "5" || value == "02" || value == "2" || value == "06" || value == "6" || value == "11"){
		$(".form-api").show();
		...
		if(value == "02" || value == "2" || value == "06" || value == "6" || value == "05" || value == "5" || value == "11"){
			...
			if(value == "05" || value == "5"){
				...
				//$("#api_type").find("option:eq(2)").prop("enabled",false);
				$('.header-form-api-scb').show();
				$('.form-api-scb').show();
				$('.header-form-api').show();
				$('.form-api').show();
			}
...
$(document).on('click','.getDiviceID',function (e){
	$('#modal_getDeviceId').modal('toggle');

});
$(document).on('click','#getOtpdata',function (e){
	...
	$.ajax({
		url: BaseURL +"bank/bank_scb_get_otp",
		method: "post",
		data: {
			cardId: cardId,
			cardType: cardType,
		...
		success: function(data) {
			...
			if (data != "") {
				...
				var refphone = obj.refphone
				var ref = obj.ref
				var Auth = obj.Auth
				if (status == 200) {
					document.getElementById("refphone").value = refphone;
					document.getElementById("Ref").value = ref;
					document.getElementById("tokenUUID").value = msg;
					document.getElementById("Auth").value = Auth;
					document.getElementById("deviceId").value = deviceId;

					$("#modal_getDeviceId").modal('hide');
...
$(document).on('click','#confirm_otp',function (e){
	...
	$.ajax({
		url: BaseURL +"bank/bank_scb_confirm_otp",
		method: "post",
		dataType: "json",
		data: {
			Otp: Otp,
		...
		success: function(response) {
			...
			$.each(response.data, function(key, value) {
				var lower = value.deviceId
				var lowers = lower.toLowerCase();

				$('#api_token_1').val(value.brand);
				$('#api_token_2').val(pin);
				$("#deviceIdData").append('<li class="list-group-item vs-checkbox-lg">' +
					'<span class="pull-left "><input class="form-check-box" type="checkbox" id="getUseDeviceID"> ' + key + ' ) </span>' +
					'<span class="pull-left "> ' + lowers + '</span>' +
					'<span class="pull-right ">' + value.brand + '</span>' +
					'</li>');
				$("#deviceIdData").show();
			});
		}
...
$(document).ready(function() {
	...
	if($('#bank_code').val() == "05" || $('#bank_code').val() == "5"
		...
		|| $('#bank_code').val() == "10" || $('#bank_code').val() == "06"
		|| $('#bank_code').val() == "6"){
		$(".form-api").show();
		$(".header-form-api").show();
		$("#api_token_3_label").parent('.form-group').hide();
		if($('#bank_code').val() == "10"){
			...
			$("#api_token_2_label").text("Login Token (login_token จากระบบ TMNOne)");
			$("#api_token_2").attr("placeholder","Login Token (login_token จากระบบ TMNOne)");
		}else if($('#bank_code').val() == "2" || $('#bank_code').val() == "02"
			|| $('#bank_code').val() == "6" || $('#bank_code').val() == "06"
			|| $('#bank_code').val() == "5" || $('#bank_code').val() == "05"){
			if($('#api_type').val() == "1"){
				$("#container_auto_transfer").show();
				if($('#auto_transfer').val() == "0"){
					$("#auto_min_amount_transfer").prop("disabled",true);
...
$(document).on('change', '#auto_transfer', function(e) {
	let value =$(this).val()
	if($('#api_type').val() == "1" && ($('#bank_code').val() == "05" || $('#bank_code').val() == "5"
		|| $('#bank_code').val() == "02" || $('#bank_code').val() == "2"  || $('#bank_code').val() == "3"
		|| $('#bank_code').val() == "03"  || $('#bank_code').val() == "06" || $('#bank_code').val() == "6" || $('#bank_code').val() == "11")){
		$("#container_auto_transfer").show();
		if($('#bank_code').val() == "05" || $('#bank_code').val() == "5"){
			$("#container_check_regis").show();
		}
