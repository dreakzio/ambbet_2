...
function dataTable() {
	...
	table = $('#table').DataTable({
		...
		// 'serverMethod': 'get',
		ajax: {
			url: BaseURL + 'user/user_list_page',
			data: function(d) {
		...
		columns: [
			...
			},
			{
				...
				render: function(data, type, full, meta) {
					...
					let view_transacntion = BaseURL + 'user/user_manage_transaction/' + id;
					...
					html += '<a class="dropdown-item"  href="' + view_transacntion +'" ><span><i class="fa fa-search mr-1" style="color:#02E5F8;"></i></span>ข้อมูลธุรกรรม</a>';
			...
			// 			username
			// 		} = full;
			// 		let update = BaseURL + 'user/user_manage_transaction/' + id;
			// 		html += '<div class="btn-group">';
			// 		html += '<button type="button" class=" btn bg-gradient-info waves-effect waves-light  dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span><i class="fa fa-edit mr-1"></i></span>จัดการข้อมูลธุรกรรม</button>';
			// 		html += '<div class="dropdown-menu animated ">'; //bounce flipInY
			// 		// html += '<a class="dropdown-item" href="detail" ><span><i class="fa fa-search mr-1" style="color:#06d79c;"></i></span>ตรวจสอบ</a>';
			// 		// html += '<div class="dropdown-divider"></div>';
			// 		html += '<a class="dropdown-item"  href="' + update + '"><span><i class="fa fa-money mr-1" style="color:#ffb22b;"></i></span>จัดการธุรกรรม</a>';
			// 		// html += '<div class="dropdown-divider"></div>';
			// 		// html += '<a class="dropdown-item btn_delete" href="javascript:void(0)" data-id="' + id + '" data-username="' + username + '"><span><i class="fa fa-trash mr-1" style="color:#ef5350;"></i></span>ลบ</a>';
			// 		html += '</div>';
			// 		html += '</div>';
		...
		.then((result) => {
			if (result.value) {
				Swal.fire({
					...
					Swal.showLoading();
				$.ajax({
					url: BaseURL + "user/user_create_agent_username/" + id,
					method: "POST",
					dataType: 'json',
