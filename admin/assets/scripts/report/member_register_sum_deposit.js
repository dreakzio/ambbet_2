...
function dataTable() {
	...
	table = $('#table').DataTable({
		...
		// 'serverMethod': 'get',
		ajax: {
			url: BaseURL + 'report/user_list_member_register_sum_deposit_page',
			data: function(d) {
		...
		columns: [
			...
			},
			{
				...
				render: function(data, type, full, meta) {
					...
					html = sum_deposit;
					if(parseFloat(sum_deposit) > 0){
						html = "<strong class='text-success'>"+numeral(sum_deposit).format('0,0.00')+"</strong>"
					}else{
						html = numeral(sum_deposit).format('0,0.00')
					}
					return html;
				}
			},
			...
			Swal.fire({
				...
				.then((result) => {
					if (result.value) {
						...
						$.ajax({
							url: BaseURL + 'report/user_list_member_register_sum_deposit_excel',
							data :params,
							method: "POST",
							dataType: 'json',
							success: function(response) {
									...
									if(response.data.length == 0){
										...
										for(let i =0;i<length_data;i++){
											...
											excel_config.data.push({
												...
												sum_deposit : data[i].sum_deposit,
											})
										}
										excel_config.filename = "รายงานยอดฝากรวมรายวัน"+(search_start_datetime!="0"  && search_start_datetime!="" ? " "+search_start_datetime.replaceAll(":",".")+" ถึง "+search_end_datetime.replaceAll(":",".") : "");
										excel_config.sheetname = "รายงานยอดฝากรวมรายวัน";
										exportExcel();
									}
								} else {
									sweetAlert2('warning', 'ทำรายการไม่สำเร็จ');
